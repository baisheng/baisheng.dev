...
module.exports = function (eleventyConfig) {
	...
	eleventyConfig.addShortcode('respimg', (path, alt, style) => {
		...
		const srcset = eleventyConfig.srcsetWidths
			.map(({ w, v }) => {
				// return `${fetchBase}dpr_auto,q_auto,w_${w}/baisheng.com/${path}.${eleventyConfig.format} ${v}w`
				return `${fetchBase}/${path}.${eleventyConfig.format}?imageMogr2/thumbnail/${w}x ${v}w`
			})
			.join(', ')
